...
<div class="container">
    <div class="row">
        ...
        <div class="col-4"  >
            ...
            <form action="{{route('form.guncel')}}" method="POST">
                @csrf

                <input hidden  name="kitap_guncel_id" value="{{$kitap->id}}">

                <div class="mb-3">
                  <label for="exampleInputEmail1" class="form-label">Kitap Adı :</label>
                  <input type="text" class="form-control" name="kitapguncel_adi" value="{{$kitap->kitap_name}}">
                </div>
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Kitap Yazarı :</label>
                    <input type="text" class="form-control" name="kitapguncel_yazari" value="{{$kitap->kitap_yazari}}">
                </div>
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Kitap Kategorisi:</label>
                    <select name="kitapguncel_kategori" class="form-select" aria-label="Default select example" >

                        <option selected> {{$kategori->kategori_name}}</option>
